...
async fn test_buffer_access_hang_reproducer() {
    ...
    let test_buffer = Arc::new(Buffer::new(
        device.clone(),
        1024,
        GPUBufferUsage::FragmentShaderRead,
        "hang_test_buffer",
        |_| TestData { x: 0.0, y: 0.0, z: 0.0 }
    ...
    port.add_fixed_pass(PassDescriptor::new("buffer_access_hang_test".to_string(), vertex_shader, fragment_shader, bind_style, DrawCommand::TriangleStrip(4), false, false)).await;
    ...
    loop {
        ...
        if start.elapsed() > max_test_duration {
            panic!("❌ Test timed out waiting for buffer access operations to complete! This indicates the hang issue is present.");
        }
    }
