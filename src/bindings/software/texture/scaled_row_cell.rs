...
impl ScaledRowCell {
    #[inline] pub const fn new(cell: u16, row: u16, scale: u8, cell_i: u8, cell_j: u8) -> Self {
        assert!(cell_i < scale);
        assert!(cell_j < scale);
        Self { cell, row, scale, cell_i, cell_j}
    }

    #[inline] pub const fn cell(&self) -> u16 {
        self.cell
    }
    ...
    #[inline] pub const fn cell_i(&self) -> u8 { self.cell_i}

    ///The position in the y dimension interior to the cell.
    ///
    /// This ranges from [0,scale)
    #[inline] pub const fn cell_j(&self) -> u8 { self.cell_j}

    ///The internal scale of the coordinate.  This is both x and y dimensioned.
    #[inline] pub const fn scale(&self) -> u8 { self.scale }
    ...
    #[inline] pub fn x_evenly(&self) -> f32 {
    ...
    ///
    /// This interpretation is evenly spaced within a given cell.  It is NOT evenly spaced across cells necessarily.
    #[inline] pub fn y_evenly(&self) -> f32 {
    ...
    ///
    /// This interpretation is evenly spaced within a given cell.  It is NOT evenly spaced across cells necessarily.
    #[inline] pub fn x_evenly_within(&self) -> f32 { (self.cell_i + 1) as f32 / (self.scale + 1) as f32 }
    ///Returns an interpretation of the x coordinate as a floating point value within the cell (e.g. in the range 0-1)
    ///
    /// This interpretation is evenly spaced within a given cell.  It is NOT evenly spaced across cells necessarily.
    #[inline] pub fn y_evenly_within(&self) -> f32 { (self.cell_j + 1) as f32 / (self.scale + 1) as f32 }
    ...
    #[inline] pub fn x_evenly_on_first(&self) -> f32 { (self.cell_i) as f32 / (self.scale) as f32}

    ///Returns an interpolation of the y coordinate as a floating point value.  The first value in a row is 'on' the row border,
    /// the last value is near the far edge.
    #[inline] pub fn y_evenly_on_first(&self) -> f32 { self.cell_j as f32 / (self.scale) as f32}

    ///Converts into a normalized coordinate.
    ///
    /// Pass in the width and height of the *source* texture.
    #[inline] pub fn into_normalized(self,width:u16,height:u16) -> Normalized {
...
*/
    #[inline] pub fn rescale_evenly(self, src_width: u16, src_height: u16,dst_width:u16,dst_height:u16) -> Scaled32 {
        ...
        let xnum = (dst_width - 1) as ExtendedInt * (self.cell * self.scale as u16 + self.cell_i as u16) as ExtendedInt;
        let xdenom = (src_width - 1) * self.scale as u16;
        let ynum = (dst_height - 1) as ExtendedInt * (self.row * self.scale as u16 + self.cell_j as u16) as ExtendedInt;
